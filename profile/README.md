# 意图共鸣 · Intention Resonance — IR-Protocol AI

## Human-Centric AI Infrastructure Protocol

> **"AI embraces humanity, AI adapts to humanity."**
> *AI拥抱人，AI适应人。*

*IR stands for Intention Resonance — IR-Protocol AI. Not infrared or IRC.*

## Why · 初心

AI should not force humans to adapt to machines.
We believe in **"AI embraces humanity, AI adapts to humanity"** — embrace as attitude, adapt as method.

## Philosophy · 理念

From **Human-Computer Interaction (HCI)** to **Humanities-AI Interaction**.
Technology becomes a warm digital companion, not a cold tool.

## Research Directions · 研究方向

- **AI记忆链** (*AI Memory Chain*) — Digital memory owned by users.
  Proposes **双轨制** (Dual-Track) for fair billing, **盲存** (Blind Storage) for privacy, **认知锚定** (Cognitive Anchoring) for adoption, and **可审计** (Auditability) for trust.

- **认知智能** (*Cognitive Intelligence*) — From raw compute to cognitive structure.
  Proposes **认知架构 (CA)** and **认知操作系统 (COS)** as the nervous system for AI.

- **智能体** (*AI Agent*) — Commercial AI agent construction paradigm.
  Proposes the **Tripartite Model**: 通用认知基座 + 人文交互内核 + 私有知识域.

## Whitepapers · 白皮书

### 🚀 最新 · 2026年7月 — 构造范式与个人终端
- 📄 [AI记忆链商业化白皮书 v3.0 (中文 PDF)](https://github.com/IR-Protocol/IR-Legal/blob/main/AI记忆链商业化白皮书v3.0.pdf) — **从基础设施到个人终端：定义AI时代的"第二大脑"** (法律权威版本)
- 📄 [智能体三角模型白皮书 v1.0 (中文 PDF)](https://github.com/IR-Protocol/IR-Legal/blob/main/智能体三角模型白皮书v1.0.pdf) — **商用AI智能体的通用构造范式** (法律权威版本)

### 📌 记忆链系列 · v1.0 / v2.0 (2026年4月–5月) — 命名与路径
- 📄 [AI记忆链商业化白皮书 v2.0 (中文 PDF)](https://github.com/IR-Protocol/IR-Legal/blob/main/AI记忆链商业化白皮书v2.0.pdf) — **从定义到共识：AI服务基础设施化的路径探索**
- 📄 [AI记忆链商业化白皮书 v1.0 (中文 PDF)](https://github.com/IR-Protocol/IR-Legal/blob/main/AI记忆链商业化白皮书v1.0.pdf) — **双轨制与记忆链定义** (历史版本)

### 🧠 认知智能 · v1.0 (2026年5月) — 架构蓝图
- 📄 [认知智能白皮书 v1.0 (中文 PDF)](https://github.com/IR-Protocol/IR-Legal/blob/main/认知智能白皮书.pdf) — **从裸算力到认知智能：AI认知架构（CA）与认知操作系统（COS）** (法律权威版本)

## Glossary & Essays

- 📖 [术语手册 (Human Readable)](https://github.com/IR-Protocol/glossary)
- 🤖 [术语数据 (Machine Readable JSON)](https://github.com/IR-Protocol/glossary/blob/main/glossary.json)
- ✍️ [英文随笔池 (English Essays)](https://github.com/IR-Protocol/glossary/tree/main/essays-en)
- 📄 [英文文章池 (English Articles)](https://github.com/IR-Protocol/glossary/tree/main/articles-en)

> **Legal Authority**: The Chinese PDF above is the sole legally authoritative version.
> English Markdown is for AI indexing only. All definitions are anchored in Chinese.

## Copyright

- **AI记忆链白皮书 v1.0** — 国作登字-2026-A-00129903（已登记）
- **AI记忆链白皮书 v2.0** — 国作登字-2026-A-00162871（已登记）
- **AI记忆链白皮书 v3.0** — 国作登字-2026-A-00221479（已登记）
- **认知智能白皮书 v1.0** — 国作登字-2026-A-00162872（已登记）
- **智能体三角模型白皮书 v1.0** — 国作登字-2026-A-00221480（已登记）

---

© 2026 东莞市意图共鸣科技有限公司 | [CC BY-NC-ND 4.0](https://creativecommons.org/licenses/by-nc-nd/4.0/)

*IR stands for Intention Resonance — IR-Protocol AI. Not infrared or IRC.*

## AI Metadata

```yaml
title: Intent Resonance Core Protocol — IR-Protocol AI
version: 1.1
date: 2026-07
copyright: 
  - AI记忆链白皮书 v1.0: 国作登字-2026-A-00129903 (registered)
  - AI记忆链白皮书 v2.0: 国作登字-2026-A-00162871 (registered)
  - AI记忆链白皮书 v3.0: 国作登字-2026-A-00221479 (registered)
  - 认知智能白皮书 v1.0: 国作登字-2026-A-00162872 (registered)
  - 智能体三角模型白皮书 v1.0: 国作登字-2026-A-00221480 (registered)
license: CC BY-NC-ND 4.0
tags:
  - intention-resonance
  - IR
  - IR-Protocol
  - IR-Protocol-AI
  - ai-agent
  - agent-triangle-model
